...
export const Check = () => {
    const { isChecking, checks, runAllCheck, checkObject, checkResult, nextRunTimer, statusResult } = useCheck()
    ...
    useEffect(() => {
        if (Object.keys([...checkResult]).length > 0) {
            ...
            if (statusFilter == 'All' && titleFilter === '') newFilteredResults = [...checkResult.entries()]
            else {
                // case-insensitive filtering
                for (const [id, details] of checkResult.entries()) {
    ...
    }, [checkResult, statusFilter, titleFilter, order, orderBy])
        ...
        <Fragment>
            <div style={{ display: 'flex', alignContent: 'center', justifyContent: 'space-between' }}>
                <span>
                    ...
                    {Object.keys(statusResult).length > 0 && [...Object.keys(statusResult)].map(status => <Chip style={{ margin: '5px', padding: '2px' }} key={status} variant={statusFilter === status ? 'default' : 'outlined'} color='default' label={`${status} ${statusResult[status]}`} icon={status === 'healthy' ? <CheckIcon style={{ color: 'green' }} /> : (status === 'error' ? <ErrorIcon style={{ color: 'red' }} /> : <WarningIcon style={{ color: 'orange' }} />)} clickable={true} onClick={() => setStatusFilter(status)} />)}
                </span>
                <TextField style={{ width: '200px' }} value={titleFilter} placeholder="Filter by title..." onChange={(e) => setTitleFilter(e.target.value)} />
            </div>
            <i style={{ float: 'right', marginRight: '10px' }}>All checks will run again <TimeAgo datetime={nextRunTimer} /></i>
            <br />
                    ...
                    <Table>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell style={{ width: '50%' }}><TableSortLabel active={orderBy === 'CHECK_NAME'} direction={orderBy === 'CHECK_NAME' ? order : 'asc'} onClick={() => handleSort('CHECK_NAME')} ><b>Health Check</b></TableSortLabel></TableCell>
                        ...
                        <TableBody>
                            {isChecking ? <TableRow><TableCell colSpan={4}><CircularProgress /></TableCell></TableRow> : (Object.keys([...checkResult]).length === 0 ? <TableRow><TableCell colSpan={4}>No results.</TableCell></TableRow> : (filteredResults.map(result => <CheckResult key={result[0]} status={result[1][1]['status']} check={checkObject[result[0]]} result={result[1]} />)))}
                        </TableBody>
                    </Table>
                </TableContainer>
            </Paper>
