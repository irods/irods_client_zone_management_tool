...
export default {
    ...
    checker: function () {
        ...
        const passThruRescIds = this.rescAll._embedded.filter(resc => resc[1] == 'passthru').map(resc => [resc[0], resc[2], resc[11], 0]);
        ...
        this.rescAll._embedded.forEach(resc => {
            ...
            if (parentId) {
                const parentResc = passThruRescIds.find(resc => resc[2] == parentId);  // find the parent resource in the list of passthru resources
                if (parentResc) { 
        ...
        if (result.failed.length > 0) {
            result.message = <span>
                The following passthru resources have either 0 or more than 1 children: {result.failed.map((resc, index) => 
                    <span key={index}>
                        {index !== 0 && ', '}
                        <Link to={`/resources?filter=${resc[0]}`}>{resc[0]}:{resc[1]}</Link> ({resc[3]} children)
                    </span>                
                )}
            </span>
        } else {
