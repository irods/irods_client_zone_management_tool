...
export const validateRescHostname = {
    ...
    "description": "A resource hostname should match a known server",
    ...
    "checker": function () {
        ...
        result.message = result.status === 'warning' ? <span><span>Failed on: </span>{result.failed.map((failedResc, index) => <span key={`rescCheckFailed-${failedResc[10]}`}>{index !== 0 && ', '}<Link className="check_result_link" to={`/resources?initalRescNameFilter=${failedResc[0]}`}>{`${failedResc[0]}:${failedResc[1]}`}</Link> ({failedResc[4]})</span>)}</span> : 'All resources have a valid hostname.'
        return result
    }
}