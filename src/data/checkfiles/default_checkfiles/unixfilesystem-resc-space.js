...
export default {
    ...
    checker: function () {
        ...
        this.rescAll._embedded.forEach(resc => {
            ...
            console.log(`freeSpaceInBytes: ${freeSpaceInBytes}`, `parsedFreeSpaceInBytes: ${parsedFreeSpaceInBytes}`)
            ...
            if (freeSpaceInGB < minimum_size_in_gb) {
                notEnoughSpaceRescs.push([name, zone, freeSpaceInBytes, freeSpaceInGB.toFixed(1)]);
                return;
            }
        })

        result.status = (invalidSpaceRescs.length > 0 || notEnoughSpaceRescs.length > 0) ? 'warning' : 'healthy';
        result.message = []

        if (invalidSpaceRescs.length > 0) {
            result.message.push(<span key="invalidSpaceRescs"><span>These resources have invalid free space values: </span>{invalidSpaceRescs.map((failedResc,index) => <span key={`invalidSpaceRescs-${index}`}>{index !== 0 && ', '}<Link className="check_result_link" to={`/resources?filter=${encodeURIComponent(failedResc[0])}`}>{failedResc[0]}:{failedResc[1]}</Link> (&ldquo;{failedResc[2]}&ldquo;)</span>)}</span>)
        } 
        ...
        if (notEnoughSpaceRescs.length > 0) {
            result.message.push(
              <span key="notEnoughSpaceRescs">
                <span>
                    {result.message.length != 0 && ". "}These resources have less than {minimum_size_in_gb} GB of free space:{" "}
                </span>
                {notEnoughSpaceRescs.map((failedResc, index) => (
                  <span key={`notEnoughSpaceRescs-${index}`}>
                    ...
                    <Tooltip title={`${failedResc[2]} bytes`} placement="top" arrow>
                        <span>
                            ({failedResc[3]} GB)
                        </span>
                    </Tooltip>
