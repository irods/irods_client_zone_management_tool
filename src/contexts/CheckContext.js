...
const validateChecks = (checks) => {
  return checks.reduce((acc, curr) => {
    ...
    assert_checkfile_precondition(
      state,
      curr.name && typeof curr.name === "string",
    );
    // `description` exists and is a string
    assert_checkfile_precondition(
      state,
      curr.description && typeof curr.description === "string",
    );
    // `interval_in_seconds` exists and is a positive number
    assert_checkfile_precondition(
      state,
      curr.interval_in_seconds &&
        typeof curr.interval_in_seconds === "number" &&
        curr.interval_in_seconds > 0,
    );
    // `checker` function exists and is a function
    assert_checkfile_precondition(
      state,
      curr.checker && typeof curr.checker === "function",
    );
    // either a `minimum_server_version` doesn't exist, or it is a string
    assert_checkfile_precondition(
      state,
      !curr.minimum_server_version ||
        typeof curr.minimum_server_version === "string",
    );
    // either a `maximum_server_version` doesn't exist, or it is a string
    assert_checkfile_precondition(
      state,
      !curr.maximum_server_version ||
        typeof curr.maximum_server_version === "string",
    );
    ...
    assert_checkfile_precondition(
      state,
      !("active" in curr) || typeof curr.active === "boolean",
    );
...
export const CheckProvider = ({ children }) => {
  ...
  const [checkIntervals, setCheckIntervals] = useState(
    localStorage.getItem("zmt-checkIntervals")
      ? JSON.parse(localStorage.getItem("zmt-checkIntervals"))
      : {},
  );
  ...
  const [inactiveChecks, setInactiveChecks] = useState(
    localStorage.getItem("zmt-inactiveChecks")
      ? new Set(JSON.parse(localStorage.getItem("zmt-inactiveChecks")))
      : new Set(
          checks.reduce((prev, check, index) => {
            if (!check.active) {
              prev.push(`zmt-${index}`);
            }
            return prev;
  ...
  const removePrevStatus = (check) => {
    ...
    setCheckResults((prev) => {
      prev[check["id"]] = [
        check,
        { status: "inactive", message: "N/A", timestamp: "N/A" },
      ];
      return prev;
    });
  ...
  const runAllCheck = useCallback(() => {
    ...
    checks.forEach((check, index) => {
      (async function () {
        ...
        newCheckIntervals[check["id"]] =
          check.id in checkIntervals
            ? checkIntervals[check.id]
            : "interval_in_seconds" in check
              ? check["interval_in_seconds"]
              : "N/A";
        // if the check is invalid
        if (!check.isValid) {
          newStatusResult["invalid"]++;
          newCheckResults[check["id"]] = [
            check,
            {
              status: "invalid",
              message: `Checkfile is invalid.`,
              timestamp: new Date(),
            },
          ];
        }
          ...
          try {
            ...
            if (checkServerVersion(check) === true) {
              ...
              if (
                ...
                newCheckTimers[check["id"]] = setTimeout(
                  () => callBackFn.current(check),
                  1000 * newCheckIntervals[check["id"]],
                );
